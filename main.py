...
# =====================================================================
# ⚙️ НАСТРОЙКИ (ТВОИ ДАННЫЕ УЖЕ ВСТАВЛЕНЫ)
# =====================================================================
...
# Твои данные ЮMoney
DEFAULT_WALLET = "4100119204482546"
DEFAULT_CLIENT_ID = "AC4879251831D5B81141A0D35CE10542A5A52F60D63ADAD9A5741FD3D49D7A31"
DEFAULT_CLIENT_SECRET = "603188CFA61B930D3E32DA9DCDB85EDA4240A2FE9292FB7440D4AE8BF6311A6CE1B27287C589508725DFEEBF2E42E5A611CC0EE5BDF78A46484BBEA90FE39D5E"

DEFAULT_API_KEY = "XVxwJNlgJr8zcuKv0us99lXa82ugRbCuFOSTT6GBUabWwXi2ucADvVaTSYvp"
DEFAULT_MARGIN = 2.0         # Наценка (коэффициент)
MIN_ORDER_COST = 5.0         # Минималка заказа в рублях
DEFAULT_SUPPORT = "https://t.me/your_support_username"  # Ссылка на поддержку
# =====================================================================
...
# --- ГЛАВНОЕ МЕНЮ (ПОЛНОСТЬЮ БЕЗ РЕКЛАМЫ) ---
...
# --- ЛОГИКА ПРОВЕРКИ СТАТУСА ЮМОНИ С ВЫВОДОМ ОШИБОК ---
...
async def refill_check(call: CallbackQuery):
    ...
    if not tok or "XXXXX" in wallet:
        return await call.answer("❌ Токен ЮMoney не задан! Перейдите в Админку -> Настройки и укажите ваш токен.", show_alert=True)
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post("https://yoomoney.ru/api/operation-history", headers=headers, data={"label": lbl}) as r:
                ...
                if status_code == 401:
                    return await call.answer("❌ Ошибка ЮMoney: Неверный токен авторизации (Unauthorized). Перевыпустите токен ЮMoney.", show_alert=True)
                elif status_code != 200:
                    return await call.answer(f"❌ Ошибка сервера ЮMoney (Код {status_code}). Попробуйте позже.", show_alert=True)
                
                d = await r.json()
                
                if "error" in d:
                    err_msg = d["error"]
                    if err_msg == "illegal_param_scope":
                        return await call.answer("❌ Ошибка прав: ваш токен не имеет разрешения 'operation-history'. Получите токен заново с галочкой истории операций!", show_alert=True)
                    return await call.answer(f"❌ Ошибка ЮMoney API: {err_msg}", show_alert=True)
                
                operations = d.get("operations", [])
                
                if not operations:
                    return await call.answer("❌ Платеж не найден. Вы точно оплатили счет в браузере? (Зачисление может занимать 1-2 минуты)", show_alert=True)
                
                target_op = None
                for op in operations:
                    if op.get("label") == lbl and op.get("status") == "success":
                        target_op = op
                        break
                ...
                if target_op:
                    async with aiosqlite.connect(DB_PATH) as db:
                        ...
                        if p:
                            if p[2] == 'success':
                                return await call.message.edit_text("✅ Этот платеж уже был зачислен ранее.")
                            ...
                            await db.commit()
                            return await call.message.edit_text(f"🎉 Баланс успешно пополнен на {p[0]}₽!")
                        else:
                            return await call.answer("❌ Запись платежа не найдена в базе бота.", show_alert=True)
                else:
                    return await call.answer("❌ Платеж в системе ЮMoney есть, но он еще обрабатывается (не статус success). Подождите минуту.", show_alert=True)
                    
    except Exception as e:
        logging.error(f"Refill error: {e}")
        await call.answer(f"❌ Ошибка соединения или базы: {str(e)[:50]}", show_alert=True)


# =====================================================================
# 👑 АДМИН-ПАНЕЛЬ (УМНЫЕ ХЭНДЛЕРЫ)
# =====================================================================

async def show_user_profile(event, target_id, state: FSMContext):
    async with aiosqlite.connect(DB_PATH) as db:
        u = await (await db.execute("SELECT balance, is_blocked, role, username FROM users WHERE user_id=?", (target_id,))).fetchone()
    if not u:
        if isinstance(event, Message):
            await event.answer("❌ Пользователь не найден в базе данных.")
        else:
            await event.answer("❌ Пользователь не найден в базе данных.", show_alert=True)
        return

    await state.update_data(t_id=str(target_id), t_role=u[2])
    ...
    text = (f"👤 <b>Пользователь:</b> <code>{target_id}</code>\n"
    ...
    if isinstance(event, Message):
        await event.answer(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    else:
...
async def show_settings_menu(event):
    ...
    yoo_token = await get_conf("yoo_token")
    
    yoo_status = "🟢 Активен" if yoo_token else "🔴 Не настроен (Нажми кнопку ниже!)"
    ...
    kb = [
        ...
        [InlineKeyboardButton(text="👛 ЮMoney кошелек", callback_data="set_wallet"), InlineKeyboardButton(text="🔐 Сделать Авто-Токен", callback_data="set_yoo_token")],
...
async def adm_u_search_res(msg: Message, state: FSMContext):
    ...
    await show_user_profile(msg, msg.text, state)
...
async def adm_u_ban(call: CallbackQuery, state: FSMContext):
    ...
    await show_user_profile(call, t_id, state)
...
async def adm_st_start_edit(call: CallbackQuery, state: FSMContext):
    ...
    if key == 'yoo_token':
        # Выдаем готовую ссылку авторизации
        auth_url = f"https://yoomoney.ru/oauth/authorize?client_id={DEFAULT_CLIENT_ID}&response_type=code&redirect_uri=https://yoomoney.ru&scope=account-info+operation-history"
        await call.message.edit_text(
            f"🔐 <b>АВТОМАТИЧЕСКАЯ ГЕНЕРАЦИЯ ТОКЕНА ЮMONEУ</b>\n\n"
            f"Тебе больше не нужны сторонние сайты! Просто сделай следующее:\n\n"
            f"1. Перейди по этой официальной ссылке:\n👉 <a href='{auth_url}'>АВТОРИЗОВАТЬСЯ В ЮMONEY</a>\n"
            f"2. Нажми «Разрешить» на сайте.\n"
            f"3. Тебя перекинет на пустую главную страницу ЮMoney.\n"
            f"4. Скопируй из адресной строки браузера **всю ссылку целиком** (или часть с <code>?code=...</code>)\n"
            f"5. **Отправь её мне (боту) ответным сообщением сюда!**",
            parse_mode="HTML",
            disable_web_page_preview=True
        )
    else:
...
# --- УМНЫЙ ОБРАБОТЧИК СОХРАНЕНИЯ НАСТРОЕК (С АВТО-ОБМЕНОМ ЮMONEУ КОДА) ---
...
async def adm_st_save(msg: Message, state: FSMContext):
    ...
    # Авто-обмен кода ЮMoney на токен прямо в боте!
    if key == 'yoo_token':
        code = val
        if "code=" in val:
            # Извлекаем код из присланной ссылки
            try:
                code = val.split("code=")[1].split("&")[0]
            except Exception:
                return await msg.answer("❌ Не удалось распознать код из ссылки. Убедись, что скопировал ссылку полностью:")
        
        await msg.answer("⏳ Связываюсь с сервером ЮMoney, совершаю обмен...")
        
        # Делаем запрос к API ЮMoney для обмена
            ...
            async with aiohttp.ClientSession() as s:
                async with s.post(url, data=payload) as r:
                    ...
                    if "access_token" in res_data:
                        real_token = res_data["access_token"]
                        async with aiosqlite.connect(DB_PATH) as db:
                            await db.execute("UPDATE settings SET value=? WHERE key='yoo_token'", (real_token,))
                            await db.commit()
                        await msg.answer("🎉 <b>ТОКЕН УСПЕШНО СГЕНЕРИРОВАН И УСТАНОВЛЕН!</b>\n\nТеперь бот может проверять платежи в фоновом режиме на 100%.", parse_mode="HTML")
                        await state.clear()
                        return await show_settings_menu(msg)
                    else:
                        err = res_data.get("error", "unknown")
                        return await msg.answer(f"❌ ЮMoney вернул ошибку обмена: <code>{err}</code>.\n\nВозможно, этот временный код уже устарел. Пожалуйста, перейди по ссылке заново, нажми «Разрешить» и пришли НОВУЮ ссылку с кодом:", parse_mode="HTML")
        except Exception as e:
            return await msg.answer(f"❌ Ошибка соединения с ЮMoney: {e}")
