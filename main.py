...
# =====================================================================
# ⚙️ НАСТРОЙКИ (ОБЯЗАТЕЛЬНО ЗАПОЛНИ ПЕРЕД ЗАПУСКОМ)
# =====================================================================
BOT_TOKEN = "8969739360:AAG-0Pt5Xbl_A5sk01AZMx2CEmtAvx6LWm8"
OWNER_ID = 7888286588  # Твой Telegram ID (числами)
...
async def order_7_final(call: CallbackQuery, state: FSMContext):
    ...
    role = await check_access(call) # Снова получаем роль, так как await db.commit() может потребовать актуализации
...
async def promo_use(msg: Message, state: FSMContext):
    ...
    role = await check_access(msg) # Обновляем роль, так как состояние изменилось
            ...
            try:
                ...
                if d.get("operations") and d["operations"][0]["status"] == "success":
                    async with aiosqlite.connect(DB_PATH) as db:
                        ...
                        if p and p[2] == 'pending': # p[2] - статус платежа из БД
...
async def adm_stats(call: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Изменен запрос для новых активных юзеров: те, кто сделал хоть один заказ за 24 часа
        ...
        # Чистая прибыль (приближенный расчет: наценка на API в среднем заложена. margin - 1.0 от стоимости заказа)
...
# 🔌 2. Модуль API Статуса (Улучшен кнопкой пополнения)
...
async def adm_api_status(call: CallbackQuery):
    ...
    await call.message.edit_text(
        ...
        f"<i>При нулевом балансе заказы пользователей автоматически уходят в 'Awaiting Refill' (очередь ожидания). Пополните счет на сайте, а затем воспользуйтесь разделом зависших заказов.</i>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=kb)
    )
...
async def adm_u_ban(call: CallbackQuery, state: FSMContext):
    ...
    # Повторный вызов adm_u_search_res для обновления сообщения
...
# (Улучшенная версия изменения баланса - теперь не теряет данные)
@router.callback_query(F.data == "u_bal")
async def adm_u_bal_start(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    if 't_id' not in data:
        return await call.answer("❌ Ошибка: сессия поиска истекла. Найдите пользователя заново через раздел 'Юзеры'.", show_alert=True)
...
async def adm_u_bal_save(msg: Message, state: FSMContext):
    ...
    if not t_id:
        await msg.answer("❌ Ошибка: сессия потеряна. Начните поиск заново в 'Юзеры'.")
    ...
    try:
        ...
        try:
            ...
            pass # Если бот не может отправить сообщение пользователю
...
async def promo_del(call: CallbackQuery):
    ...
    await promo_list(call) # Обновляем список промокодов
...
async def adm_bc_exec(msg: Message, state: FSMContext):
    ...
    for u in users:
        try:
            ...
            sent += 1
            await asyncio.sleep(0.05)  # Плавность от флуд-фильтров
        except Exception: 
            pass
...
async def adm_st_save(msg: Message, state: FSMContext):
    ...
    await state.clear()
    # Обновляем сообщение с настройками после сохранения
...
async def adm_maint_toggle(call: CallbackQuery):
    ...
    # Обновляем сообщение с настройками после изменения статуса тех. работ
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    # Сначала проверяем доступ к боту
